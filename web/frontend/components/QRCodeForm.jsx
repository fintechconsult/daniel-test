...
export function QRCodeForm({ QRCode: InitialQRCode }) {
    ...
    */
    const {
        ...
        fields: {
            ...
            // img: useField(QRCode?.img || '')
            img: useField(QRCode?.img || '')
        },
        onSubmit,
    })
    const [imgPath, setImgPath] = useState(img.value)
    const [crop, setCrop] = useState(img.value !== '' ? true : false)
    ...
    */
    ...
    const handleRemove = async () => {
        ...
        const QRCodeId = QRCode?.id
        /* construct the appropriate URL to send the API request to based on whether the QR code is new or being updated */
        const url = `/api/qrcodes/${QRCodeId}/image`
        const body = {
            img: ''
        }
        await fetch(url, {
    ...
    const handleSave = async () => {
        if (editor) {
            let n = editor.getCroppingRect()
            n = {
                // height: n.height * imageSize,
                // width: n.width * imageSize,
                // x: n.x * imageSize,
                // y: n.y * imageSize
                height: 1, width: 0.6, x: 0.3, y: 0.5
            }
            setPosition(n)
            setCrop(true)
            const QRCodeId = QRCode?.id
    ...
    const uploadImage = (event) => {
        ...
        const url = 'http://localhost/ACGPN_traindata/shopify/' + event.target.files[0].name
        setImgPath(url)
        setCrop(false)
    }
                    ...
                    crop ? (<Layout.Section >
                        <Card>
                            <EmptyState>
                                ...
                                />
                                <div>
                                    <Button destructive onClick={handleRemove}>
                                        Remove
                                    </Button>
                                </div>
                            </EmptyState>
